...
- **本机环境事实**（已实测，2026-09-23）：
  - `cargo` / `rustup` / `rustc` / `gradle` / `adb` / `gh` 全部未安装；Java 只有 8（构建 JDK 需要 17）。**本机没有 Rust 与 Android 工具链**。所有「运行命令并检查输出」的步骤都依赖 Task 0 装好的本机 Rust；APK 的编译与验证依赖 CI
  - **shell 在 `workspace-write` 沙箱下完全不可用**：任何 pwsh 命令都以 `SetNamedSecurityInfoW failed (Win32 5): grantWrite(<workspace>)` 失败，与命令内容无关。文件工具（read / write / edit / glob / grep）在该模式下**正常**。两种解法：把会话切到 `danger-full-access`（实测 shell 正常），或每条命令申请一次提权。这不是命令写错，遇到就立即提权，不要去改写命令绕
